...
export default function KanbanBoard({ boardId }: { boardId: string }) {
  ...
  // Determine if the active drag is a card or column
  const renderOverlay = () => {
    if (!activeId) return null;

    // Card overlay
    if (board.cards[activeId]) {
      return <Card boardId={boardId} cardId={activeId} />;
    }
    ...
    if (col) {
      return (
        <div className="w-72 p-4 bg-blue-100 border-2 border-blue-400 rounded-xl shadow-lg">
          {col.title}
        </div>
      );
    }
  ...
  return (
    <div>
      ...
      >
        ...
        {/* Single DragOverlay for both cards & columns */}
