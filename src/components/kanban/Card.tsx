...
export default function Card({ boardId, cardId }: CardProps) {
  ...
  // Early bailout AFTER hooks
  if (!card) {
    return (
      <div
        ref={setNodeRef}
        style={{ transform: CSS.Transform.toString(transform), transition }}
        className="p-3 rounded-lg bg-gray-100 text-gray-400"
      >
        Missing card
      </div>
    );
  ...
  return (
    <>
      <div
        ref={setNodeRef}
        style={{ transform: CSS.Transform.toString(transform), transition }}
        className={`relative p-5 rounded-2xl shadow-md hover:shadow-lg bg-white flex flex-col gap-3 border border-gray-200 ${
      ...
      >
        ...
        {/* Edit Button */}
        <button
          className="absolute top-3 right-3 p-1.5 rounded-full bg-blue-100 text-blue-600 hover:bg-blue-200"
